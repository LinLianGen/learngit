...
public class Admin extends BaseModel<Admin> {
	...
	select.append(" select admin.taday_num,admin.before_time,admin.`password`,admin.`userRole`,admin.`userName`,admin.U_id,role.`roleName`,role.roleId ");
	...
	select.append(" where admin.`isDel`=0 and admin.userName = ? ");
    ...
    public boolean updateBefore(String before_time , int taday_num , String userName){
    		StringBuffer sql = new StringBuffer();
    		sql.append(" update  ");
    		sql.append( this.tableName() );
    		sql.append("   set before_time = ?  ,  taday_num = ? ");
    		sql.append(" where  userName = ?  ");
    		int i  = 0; 
    		try {
    			i = 	Db.update(sql.toString(), before_time , taday_num, userName  );
