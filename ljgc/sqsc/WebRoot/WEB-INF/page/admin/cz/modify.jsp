							...
							<div class="layui-form-item">
								...
								<div class="layui-input-block">
									<input type="text" name="money"
										<c:if test="${op eq 'see'}">disabled="disabled"</c:if>
										value="${v.money}" placeholder="请输入充值金额名称" required
										lay-verify="required" autocomplete="off" class="layui-input">
								</div>
							</div>

							<div class="layui-form-item">
								<label class="layui-form-label">0未付款 1，已支付 </label>
								<div class="layui-input-block">
									<input type="text" name="status"
										<c:if test="${op eq 'see'}">disabled="disabled"</c:if>
										value="${v.status}" placeholder="请输入0未付款  1，已支付  名称" required
										lay-verify="required" autocomplete="off" class="layui-input">
								</div>
							</div>

							<div class="layui-form-item">
								<label class="layui-form-label">1.支付宝支付 2.微信支付 3.线下</label>
								<div class="layui-input-block">
									<input type="text" name="pay_type"
										<c:if test="${op eq 'see'}">disabled="disabled"</c:if>
										value="${v.pay_type}" placeholder="请输入1.支付宝支付 2.微信支付 3.线下名称"
							...
							<div class="layui-form-item">
								...
								<div class="layui-input-block">
									<input type="text" name="obm"
										<c:if test="${op eq 'see'}">disabled="disabled"</c:if>
										value="${v.obm}" placeholder="请输入交易订单号名称" required
										lay-verify="required" autocomplete="off" class="layui-input">
								</div>
							</div>
