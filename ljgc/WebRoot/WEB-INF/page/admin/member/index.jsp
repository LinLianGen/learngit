	...
	<script type="text/javascript">
		...
		var columns2=[{
			...
			formatter : function(value, row, index){
				var v='';
				if(row.isDel=='-1'){
					v += '<a onclick="operationMember(\''+value+'\',0)" title="启用"><i class="glyphicon glyphicon-ok-circle"></i></a> ';
				}else{
				v += '<a onclick="operationMember(\''+value+'\',-1)" title="禁用"><i class="glyphicon glyphicon-remove-circle"></i></a> ';
				}		
				return operator(true, false, false,value,row.type)+v;
			}
		}];
		tableInitPanel(columns.concat(columns2),"member/showList");
	</script>
